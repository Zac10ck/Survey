...
describe("Translation Scanner", () => {
  describe("stripComments", () => {
    ...
    test("preserves URLs with // in them", () => {
      const content = `
        const link = "https://survey.medascloud.com";
        const link2 = "http://example.com";
        const text = t("valid.key");
      `;
      const result = stripComments(content);
      expect(result).toContain("https://survey.medascloud.com");
      expect(result).toContain("http://example.com");
      expect(result).toContain('t("valid.key")');
    });

    test("preserves translation keys on same line as URLs", () => {
      const content = `<Link href="https://survey.medascloud.com">{t("s.create_your_own")}</Link>`;
      const result = stripComments(content);
    ...
    test("removes actual comments but preserves URLs", () => {
      const content = `
        const url = "https://survey.medascloud.com";
        ...
        export function ComplexComponent() {
          ...
          return (
            <div>
              ...
              <Link href="https://survey.medascloud.com">{t("page.link")}</Link>
